...
export function ProgressEditable(props: EditableState<number> & {
	...
	const changeCB = useStableCallback((e: ChangeEvent<HTMLInputElement>) => {
		finalize()
	...
	const editor = <input type="range" className="datacore-progress-editable" value={val.current} onChange={changeCB} />

	return (
			<span className="has-texteditable" >
				<Editable<number> dispatch={props.dispatch} editor={editor} state={{...props, content: val.current, isEditing: props.isEditing}} defaultRender={readOnly} />
			</span>
		)
}