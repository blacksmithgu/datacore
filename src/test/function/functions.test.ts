...
import { parseEval, simpleEvaluator } from "test/common";
...
test("Evaluate length(array)", () => {
    expect(DefaultFunctions.length(simpleEvaluator(), [1, 2])).toEqual(2);
    expect(DefaultFunctions.length(simpleEvaluator(), [])).toEqual(0);
});

test("Evaluate length(string)", () => {
    expect(DefaultFunctions.length(simpleEvaluator(), "hello")).toEqual(5);
    expect(DefaultFunctions.length(simpleEvaluator(), "no")).toEqual(2);
    expect(DefaultFunctions.length(simpleEvaluator(), "")).toEqual(0);
});
...
test("Evaluate nonnull()", () => {
    expect(DefaultFunctions.nonnull(simpleEvaluator(), null, null, 1)).toEqual([1]);
    expect(DefaultFunctions.nonnull(simpleEvaluator(), "yes")).toEqual(["yes"]);
});
