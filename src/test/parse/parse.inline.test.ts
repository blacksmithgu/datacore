...
import {
    ...
    setEmojiShorthandCompletionField,
    setInlineField,
...
test("Parse commas inside inline field", () => {
    expect(PRIMITIVES.inlineField.tryParse("[[yes, no, and maybe]]")).toEqual(Link.file("yes, no, and maybe"));
});
...
describe("Inline Field Values", () => {
    test("Parse basic inline fields", () => {
        expect(PRIMITIVES.inlineField.tryParse("14")).toEqual(14);
        expect(PRIMITIVES.inlineField.tryParse('"yes,"')).toEqual("yes,");
        expect(PRIMITIVES.inlineField.tryParse("[[test]]")).toEqual(Link.file("test"));
    });

    test("Parse inline field lists", () => {
        expect(PRIMITIVES.inlineField.tryParse("[[test]],")).toEqual([Link.file("test")]);
        expect(PRIMITIVES.inlineField.tryParse("[[test]], [[test2]]")).toEqual([Link.file("test"), Link.file("test2")]);
        expect(PRIMITIVES.inlineField.tryParse('1, 2, 3, "hello"')).toEqual([1, 2, 3, "hello"]);
    });

    test("Parse inline booleans", () => {
        expect(PRIMITIVES.inlineField.tryParse("true")).toEqual(true);
        expect(PRIMITIVES.inlineField.tryParse("False")).toEqual(false);
    });
});
